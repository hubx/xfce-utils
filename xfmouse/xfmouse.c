...
MouseDialog *mouse_dialog_new(void)
{
...
void set_defaults(MouseDialog * md)
{
...
void read_config(MouseDialog * md)
{
    FILE *fp;
    char buf[MAXSTRLEN];
...
gboolean write_config(MouseDialog * md)
{
    FILE *fp;
    const char *home = g_getenv("HOME");
    char *rcfile = g_strconcat(home, "/.xfce4/", RCFILE, NULL);
...
void set_mouse_values(MouseDialog * md)
{
...
static void dialog_response_cb(GtkWidget * d, int response, MouseDialog * md)
{
...
static void dialog_close_cb(GtkWidget * d, MouseDialog * md)
{
    gtk_main_quit();
}

static void radio_button_toggled_cb(GtkWidget * cb, MouseDialog * md)
{
    md->radiovalue = md->radiovalue ? 0 : 1;
}


GtkWidget *create_image_from_xpm_data(const char **data)
{
...
void add_header(const char *text, GtkBox *vbox)
{
    GtkWidget *label;
    char *markup;
...
void create_dialog(MouseDialog * md)
{
    ...
    gtk_box_set_spacing(GTK_BOX(GTK_DIALOG(md->dialog)->vbox), 3);
    ...
    gtk_box_pack_start(GTK_BOX
                       (GTK_DIALOG(md->dialog)->vbox), vbox, TRUE, TRUE, 0);
    ...
    add_header(_("Button Settings"), GTK_BOX(vbox));
    ...
    rb_left = gtk_radio_button_new_with_mnemonic(NULL, _("_Left handed"));
    md->rb_right =
        gtk_radio_button_new_with_mnemonic_from_widget
        (GTK_RADIO_BUTTON(rb_left), _("_Right handed"));
    ...
    gtk_size_group_add_widget(sg, rb_left);
    gtk_size_group_add_widget(sg, md->rb_right);
    ...
    add_header(_("Motion Settings"), GTK_BOX(vbox));
...
*/
    ...
    gtk_misc_set_alignment(GTK_MISC(thresh_label), 0, 0.8);
    gtk_box_pack_start(GTK_BOX(hbox1), thresh_label, FALSE, FALSE, 0);

    gtk_size_group_add_widget(sg, thresh_label);

    md->threshold = gtk_hscale_new_with_range(THRESH_MIN, THRESH_MAX, 1);
    gtk_range_set_value(GTK_RANGE(md->threshold), md->threshvalue);
    gtk_box_pack_start(GTK_BOX(hbox1), md->threshold, TRUE, TRUE, 0);
